...
class ChatService {
  ...
  Future<void> sendMessage(String channelId, ChatMessage message) async {
    ...
    try {
      // 2. Dispatch to API.
      // CUTOVER: canonical /channels-v2 message endpoint. Its schema is strict —
      // it accepts only { body, isOfficial? } (author is derived from the token),
      // so we send the processed text as `body` instead of the legacy payload.
      final response = await ApiService.post('/channels-v2/$channelId/messages', {
        'body': processedText,
