...
class Channel {
  ...
  factory Channel.fromMap(Map<String, dynamic> map, String id) {
    // Accepts both the legacy Firestore shape (camelCase) and the canonical
    // Postgres /channels-v2 shape (snake_case: is_read_only, type).
    return Channel(
      id: id,
      name: map['name'] ?? '',
      description: map['description'] ?? '',
      isReadOnly: map['isReadOnly'] ?? map['is_read_only'] ?? false,
      allowedRoles: List<String>.from(map['allowedRoles'] ?? ["resident", "admin"]),
      moderatorIds: List<String>.from(map['moderatorIds'] ?? []),
      smartType: map['smartType'] ?? map['type'],
...
class ChatMessage {
  ...
  factory ChatMessage.fromMap(Map<String, dynamic> map, String id) {
    ...
    // Timestamp parsing tolerates both a Firestore Timestamp (legacy) and an
    // ISO-8601 string (Postgres /channels-v2 `created_at`).
    ...
    return ChatMessage(
      ...
      // v2 messages carry `body`; legacy Firestore carries `text`.
      text: map['text'] ?? map['body'] ?? '',
      senderId: map['senderId'] ?? map['author_id'] ?? '',
      senderName: map['senderName'] ?? map['author_name'] ?? 'Unknown',
      senderFlat: map['senderFlat'] ?? '',
      createdAt: createdAt,
      isOfficial: map['isOfficial'] ?? map['is_official'] ?? false,
      isSystemMessage: map['isSystemMessage'] ?? false,
      isDeleted: map['isDeleted'] ?? (map['deleted_at'] != null),
