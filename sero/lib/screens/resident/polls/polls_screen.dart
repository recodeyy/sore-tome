...
class _PollCardState extends ConsumerState<_PollCard> {
  String? selectedOptionId; // v2 option UUID
  bool isVoting = false;
  bool _votedLocally = false;

  // True once the requester has voted (server-reported on load, or just now).
  bool get hasVoted => widget.poll.hasVoted || _votedLocally;

  Future<void> _castVote() async {
    if (selectedOptionId == null || isVoting) return;
    setState(() => isVoting = true);

    try {
      // Canonical v2 vote: POST /polls-v2/:id/vote with { optionId } → 201.
      final res = await ApiService.post(
        '/polls-v2/${widget.poll.id}/vote',
        {'optionId': selectedOptionId},
      );
      if (res.statusCode == 201 || res.statusCode == 200) {
        setState(() => _votedLocally = true);
  ...
  Widget build(BuildContext context) {
    final totalVotes = widget.poll.pollOptions
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Options from the v2 poll options (UUID id + label + tally).
          ...widget.poll.pollOptions.map((option) {
            final voteCount = option.count;
            final percent =
          ...
          if (!hasVoted && widget.poll.isActive) ...[
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed:
                    selectedOptionId == null || isVoting ? null : _castVote,
