...
router.get("/unread/count", authMiddleware, tenantMiddleware, async (req: Request, res: Response) => {
  try {
...
router.post("/", authMiddleware, tenantMiddleware, canManageContent, validate(CreateNoticeSchema), async (req: Request, res: Response) => {
  try {
