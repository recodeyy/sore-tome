...
/**
 * Returns true only when BOTH Razorpay keys are present in the environment.
 * Endpoints should call this and return a clean 503 (feature-disabled) when it
 * is false, instead of attempting a live gateway call with placeholder keys.
 */
export function isRazorpayConfigured(): boolean {
  return Boolean(process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET);
}

/** True when the configured key id is a Razorpay TEST key (rzp_test_…). */
