...
export const ComplaintService = {
  async createComplaint(
    ...
    return withTx(async (client) => {
      ...
      // Cross-role: notify society admins/committee of the new complaint.
      ...
      for (const userId of admins) {
        await client.query(
          `INSERT INTO notifications (society_id, user_id, title, body, type, data)
           VALUES ($1,$2,$3,$4,'complaint',$5)`,
          [societyId, userId, `New complaint ${ref}`, input.title,
           JSON.stringify({ complaintId: rows[0].id, ref, priority, deepLink: `sero://complaints/${rows[0].id}` })]
        );
      }

      logger.info({ societyId, complaintId: rows[0].id, ref, priority, dueAt: due, admins: admins.length }, "Complaint created + admins notified");
      return rows[0];
    });
  },
  ...
  async assign(
    ...
    return withTx(async (client) => {
      ...
      await Recipients.fanOut(client, {
        ...
        payload: { id: complaintId, ref: rows[0].ref, assigneeId, assigneeType },
        recipients,
        notification: {
          title: `Complaint assigned to you`,
          body: `${rows[0].ref}: ${rows[0].title}`,
          type: "complaint",
          data: { complaintId, ref: rows[0].ref, deepLink: `sero://complaints/${complaintId}` },
        },
      });
      logger.info({ societyId, complaintId, assigneeId, assigneeType, notified: recipients.length }, "Complaint assigned + staff notified");
      return upd.rows[0];
    });
  },
  ...
  async changeStatus(
    ...
    return withTx(async (client) => {
      ...
      // Cross-role: notify the resident who raised the complaint of the new status
      // (public timeline). 'resolved' gets a tailored message.
      const recipients = c.created_by ? [c.created_by] : [];
      const resolved = to === "resolved";
      await Recipients.fanOut(client, {
        societyId,
        eventType: resolved ? "complaint.resolved" : "complaint.status_changed",
        payload: { id: complaintId, ref: c.ref, from, to },
        recipients,
        notification: {
          title: resolved ? `Complaint resolved` : `Complaint updated`,
          body: resolved
            ? `${c.ref} has been marked resolved.${note ? ` ${note}` : ""}`
            : `${c.ref} is now ${to.replace("_", " ")}.`,
          type: "complaint",
          data: { complaintId, ref: c.ref, status: to, deepLink: `sero://complaints/${complaintId}` },
        },
      });

      logger.info({ societyId, complaintId, from, to, reopen, notified: recipients.length }, "Complaint status changed + resident notified");
      return upd.rows[0];
    });
  },
  ...
  async addComment(
    ...
    return withTx(async (client) => {
      const owns = await client.query(
        `SELECT id, ref, created_by FROM complaints WHERE id = $1 AND society_id = $2`,
        [complaintId, societyId]
      );
      const complaint = owns.rows[0];
      if (!complaint) throw err("Complaint not found", "NOT_FOUND");
      const { rows } = await client.query(
      ...
      // Cross-role: a resident-visible update/proof notifies the resident who
      // raised it (skip if the resident authored it themselves).
      if (input.visibility === "resident" && complaint.created_by && complaint.created_by !== input.authorId) {
        await Recipients.fanOut(client, {
          societyId,
          eventType: "complaint.commented",
          payload: { id: complaintId, ref: complaint.ref },
          recipients: [complaint.created_by],
          notification: {
            title: `Update on ${complaint.ref}`,
            body: input.body,
            type: "complaint",
            data: { complaintId, ref: complaint.ref, deepLink: `sero://complaints/${complaintId}` },
          },
        });
      }
      return rows[0];
