...
/** Look up an amenity's name for a friendly notification body (tenant-scoped). */
async function amenityName(client: any, societyId: string, amenityId: string): Promise<string> {
  const { rows } = await client.query(
    `SELECT name FROM amenities WHERE id = $1 AND society_id = $2`,
    [amenityId, societyId]
  );
  return rows[0]?.name || "amenity";
}
...
export const BookingService = {
  ...
  async book(societyId: string, input: { amenityId: string; memberId?: string; startAt: string; endAt: string }) {
    return withTx(async (client) => {
      ...
      // Cross-role: notify society admins of the new booking + confirm to the
      // resident who booked it (member_id holds the booking user's id).
      const name = await amenityName(client, societyId, input.amenityId);
      const admins = await Recipients.adminUserIds(client, societyId);
      const resident = booking.member_id ? [booking.member_id] : [];
      const deepLink = `sero://amenities/${booking.id}`;
      await Recipients.fanOut(client, {
        societyId,
        eventType: "amenity.booked",
        payload: { id: booking.id, amenityId: input.amenityId, status: booking.status, memberId: booking.member_id },
        recipients: admins,
        notification: {
          title: `New ${name} booking`,
          body: `A ${name} slot was booked.`,
          type: "amenity",
          data: { bookingId: booking.id, amenityId: input.amenityId, status: booking.status, deepLink },
        },
      });
      if (resident.length) {
        await Recipients.fanOut(client, {
          ...
          notification: {
            title: `${name} booked`,
            body: `Your ${name} booking is confirmed.`,
            type: "amenity",
            data: { bookingId: booking.id, amenityId: input.amenityId, status: booking.status, deepLink },
          },
        });
      }
      logger.info({ societyId, bookingId: booking.id, amenityId: input.amenityId, admins: admins.length, resident: resident.length }, "Amenity booked + notified");
      return booking;
    });
  },
  ...
  async cancel(societyId: string, bookingId: string) {
    return withTx(async (client) => {
      ...
      if (rows.length === 0) throw aerr("Booking not found or not active", "NOT_FOUND");
      const booking = rows[0];

      // Cross-role: notify the resident who booked + society admins of the cancel.
      const name = await amenityName(client, societyId, booking.amenity_id);
      const admins = await Recipients.adminUserIds(client, societyId);
      const resident = booking.member_id ? [booking.member_id] : [];
      const deepLink = `sero://amenities/${booking.id}`;
      await Recipients.fanOut(client, {
        societyId,
        eventType: "amenity.cancelled",
        payload: { id: booking.id, amenityId: booking.amenity_id, status: "cancelled", memberId: booking.member_id },
        recipients: Array.from(new Set([...resident, ...admins])),
        notification: {
          title: `${name} booking cancelled`,
          body: `A ${name} booking was cancelled.`,
          type: "amenity",
          data: { bookingId: booking.id, amenityId: booking.amenity_id, status: "cancelled", deepLink },
        },
      });
      logger.info({ societyId, bookingId, notified: resident.length + admins.length }, "Amenity booking cancelled + notified");
      return booking;
    });
  },
  ...
  async requestBooking(
    ...
    return withTx(async (client) => {
      ...
      // Cross-role: notify society admins of the new booking + confirm to the
      // resident who booked it. eventType reflects whether approval is pending.
      const admins = await Recipients.adminUserIds(client, societyId);
      const resident = booking.member_id ? [booking.member_id] : [];
      ...
      await Recipients.fanOut(client, {
        societyId,
        eventType: "amenity.booked",
        payload: { id: booking.id, amenityId: input.amenityId, status, memberId: booking.member_id },
        recipients: admins,
        notification: {
          title: pending ? `${amenity.name} booking awaiting approval` : `New ${amenity.name} booking`,
          body: pending ? `A ${amenity.name} booking needs your approval.` : `A ${amenity.name} slot was booked.`,
          type: "amenity",
          data: { bookingId: booking.id, amenityId: input.amenityId, status, deepLink },
        },
      });
      if (resident.length) {
        await Recipients.fanOut(client, {
          ...
          recipients: resident,
          notification: {
            title: pending ? `${amenity.name} booking requested` : `${amenity.name} booked`,
            body: pending ? `Your ${amenity.name} booking is awaiting approval.` : `Your ${amenity.name} booking is confirmed.`,
            type: "amenity",
            data: { bookingId: booking.id, amenityId: input.amenityId, status, deepLink },
          },
        });
      }
      logger.info({ societyId, bookingId: booking.id, status, admins: admins.length, resident: resident.length }, "Booking requested + notified");
      return booking;
    });
  },
  ...
  async approveBooking(societyId: string, bookingId: string) {
    return withTx(async (client) => {
      ...
      // Cross-role: notify the resident who booked that it is now confirmed.
      const name = await amenityName(client, societyId, booking.amenity_id);
      const resident = booking.member_id ? [booking.member_id] : [];
      await Recipients.fanOut(client, {
        societyId,
        eventType: "amenity.approved",
        payload: { id: booking.id, amenityId: booking.amenity_id, status: "confirmed", memberId: booking.member_id },
        recipients: resident,
        notification: {
          title: `${name} booking approved`,
          body: `Your ${name} booking has been approved.`,
          type: "amenity",
          data: { bookingId: booking.id, amenityId: booking.amenity_id, status: "confirmed", deepLink: `sero://amenities/${booking.id}` },
        },
      });
      logger.info({ societyId, bookingId, resident: resident.length }, "Booking approved + resident notified");
      return booking;
    });
  },
  ...
  async cancelBooking(societyId: string, bookingId: string, withRefund = false) {
    return withTx(async (client) => {
      ...
      // Cross-role: notify the resident who booked + society admins of the
      // cancellation (and refund, if any).
      const name = await amenityName(client, societyId, booking.amenity_id);
      const admins = await Recipients.adminUserIds(client, societyId);
      const resident = booking.member_id ? [booking.member_id] : [];
      await Recipients.fanOut(client, {
        societyId,
        eventType: "amenity.cancelled",
        payload: { id: booking.id, amenityId: booking.amenity_id, status: "cancelled", memberId: booking.member_id, refunded: withRefund },
        recipients: Array.from(new Set([...resident, ...admins])),
        notification: {
          title: `${name} booking cancelled`,
          body: withRefund ? `A ${name} booking was cancelled and refunded.` : `A ${name} booking was cancelled.`,
          type: "amenity",
          data: { bookingId: booking.id, amenityId: booking.amenity_id, status: "cancelled", refunded: withRefund, deepLink: `sero://amenities/${booking.id}` },
        },
      });
      logger.info({ societyId, bookingId, withRefund, notified: resident.length + admins.length }, "Booking cancelled + notified");
      return booking;
    });
  },
