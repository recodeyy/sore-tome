...
// DEPRECATION: this legacy router is superseded by the canonical complaints router (/complaints).
// Log every hit so we can track remaining legacy-client traffic before removal.
router.use((req, res, next) => {
  logger.warn({ path: req.originalUrl }, "DEPRECATED legacy route hit — use v2");
  next();
});
