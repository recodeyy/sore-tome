...
- **9 Society profile ✅** (society_profiles get/upsert: reg no, address, tz, currency, FY, contacts) · **10 Logo/branding ✅** (society_logos versioned set/replace/delete) · **11 Wings ✅ · 12 Blocks ✅ · 13 Floors ✅ · 14 Units ✅ · 16 Occupancy history ✅** · **15 Bulk import ✅** (dry-run/validation/dup-detect; units + members) · **17 Member lifecycle ✅** (MemberService register→approve→suspend) · **18 Family/co-owners ✅ · 19 Committee ✅ · 20 KYC workflow ✅** (wired in /members-v2) · **21 Society settings ✅** (jsonb per-section merge) · **22 Onboarding checklist ✅** (setupProgress %/blockers across profile/structure/members/finance)

### C. Finance, billing, payments, accounting (23–42) — ✅ Mostly done
- 23 Chart of accounts ✅ · 24–26 Billing config/templates/bulk gen ✅ · 27 Draft/publish/cancel ✅ · **28 Recurring billing ✅** (idempotent per society+policy+period) · **29 Proration ✅** (prorateMinor, partial-period) · **30 Late fees/waivers ✅** (idempotent apply + reversal) · **31 GST invoices/credit notes ✅** (per-line tax breakdown + immutable CN numbering) · 32 Razorpay order ✅ · 33 Webhook idempotency ✅ · 34 Payment allocation ✅ · **35 Receipts ✅** (auto-issue on capture, void/reissue) · 36 Double-entry ledger ✅ · 37 Expense capture ✅ · 38 Expense approval (maker-checker) ✅ · 39 Receipt OCR ✅ (AI) · **40 Bank reconciliation ✅** (BankAccounts + statement import + auto-match by amount + manual match + summary; tested) · 41 Dues/ageing ✅ · **42 Reports/export ✅** (CSV export with formula-injection safety)
...
- **70 Staff CRUD ✅ · 71 Permissions/restricted access ✅** (permissions[] + hasPermission) **· 72 Attendance check-in/out (no-dup) ✅ · 73 Shift/roster/conflict ✅ · 74 Leave types/balance/approve (race-safe) ✅ · 75 Payroll run (idempotent + maker-checker) ✅ · 76 Overtime/holiday/late calc ✅** (society_holidays + computeOvertimeAndLate) **· 77 KYC/contracts/certifications/expiry ✅** (staff_documents + expiringDocuments reminder) **· 78 Incidents/disciplinary ✅ · 79 Reports ✅** (attendance/leave/payroll/overtime/staffing + CSV-safe export)
...
- **86 Slot inventory/EV/accessible ✅ · 87 Vehicle registry/allocation/transfer/release/waitlist ✅ · 88 Visitor parking/violations/fines ✅** · **89 Asset registry ✅ · 90 PM schedules/work orders (no-dup)/breakdowns/AMC/downtime/parts ✅** · **91 Report templates/scheduled/exports ✅** (templates/jobs/schedules + CSV artifact generation, retention/expiry, access-controlled download) · **92 Immutable audit/access logs ✅** (Postgres audit_logs append-only via triggers; filtered read + CSV export that self-audits)
